...
class SearchPage extends StatelessWidget {
    ...
    return Column(
      children: [
        (context.watch<SearchPageCubit>().listItemCount != null &&
                list.isNotEmpty)
            ? Text(
                "${LocaleKeys.SearchPage_result.locale} ${context.watch<SearchPageCubit>().listItemCount}")
            : Container(),
        Expanded(
          flex: 8,
          child: Scrollbar(
  ...
  AppBar get buildAppBar => AppBar(
        title: Text(LocaleKeys.SearchPage_appBarTitle.locale),
        centerTitle: true,
      );
          ...
          children: [
            ...
            Expanded(
              ...
              child: Column(
                ...
                children: [
                  Text(
                      "${LocaleKeys.SearchPage_id.locale}: " + (item.id ?? "")),
                  Text("${LocaleKeys.SearchPage_name.locale}: " +
                      (item.name ?? "")),
                  Text("${LocaleKeys.SearchPage_symbol.locale}: " +
                      (item.symbol ?? ""))
                ],
              ),
            ),
